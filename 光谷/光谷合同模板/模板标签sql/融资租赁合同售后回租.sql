...
--CELL_PHONE  乙方（自然人）手机
 ...
 -----------------------------------------------------------------------------------           
     --租赁物投保
select ppli.insure_type  as INSURE_TYPE
  from prj_project_lease_item_lv ppli
...
--租赁物清单   汽车回租  先不要，待确定，因为有问题
   select ccli.full_name,  
          ccli.specification,
          ccli.license_plate_number,
          ccli.engine,   
          ccli.quantity,   
           ccli.price,     
           ccli.description   
 ...
 --买卖合同编号
  select CONTENT_NUM as SALES_CONTRACT_NUMBER
    from con_contract_content CCC
   where templet_id =
         (select CCT.TEMPLET_ID
            from con_clause_templet cct
           where cct.templet_code = 'SALES_CONTRACT') --买卖合同编号
...
select CONTENT_NUM as CONTRACT_NUMBER1
 ...
 where content_id = {$CONTENT_ID$}
  ...
  --买卖合同编号
  select CONTENT_NUM as SALES_CONTRACT_NUMBER1
    from con_contract_content CCC
   where templet_id =
         (select CCT.TEMPLET_ID
            from con_clause_templet cct
...
select ccb.bp_name as TENANT_BP_NAME5
 ...
 where ccb.bp_category = 'TENANT' 
   ...
   --买卖合同编号
  select CONTENT_NUM as SALES_CONTRACT_NUMBER3
    from con_contract_content CCC
   where templet_id =
         (select CCT.TEMPLET_ID
            from con_clause_templet cct
...
--法定代表人  
 ...
 --承租人
 select ccb.bp_name as TENANT_BP_NAME6
  from con_contract_bp_lv ccb 
 where ccb.bp_category = 'TENANT' 
   and ccb.contract_id =  {$CONTRACT_ID$}
   and rownum =1
   ...
   --承租人法定代表人
select hbm.legal_person as TENANT_ORG_LEGAL_PERSON6
  from hls_bp_master_lv hbm
 where hbm.bp_id = (select ccb.bp_id
                      from CON_CONTRACT_BP_LV ccb
                     where ccb.bp_category = 'TENANT'
...
--法定代表人  
 ...
 where employee_id = (select employee_id
  ...
  --承租人法定代表人
select hbm.legal_person as TENANT_ORG_LEGAL_PERSON7
  from hls_bp_master_lv hbm
 where hbm.bp_id = (select ccb.bp_id
                      from CON_CONTRACT_BP_LV ccb
                     where ccb.bp_category = 'TENANT'

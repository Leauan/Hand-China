CREATE OR REPLACE VIEW con_lease_service_report_lv AS
SELECT (SELECT c.contract_number
       ...
       t1.quantity, -- 数量
       t1.estimated_value, -- 设备评估值
       t1.fixed_assets_site, -- 存放地点
       t1.description, -- 备注
       t1.project_lease_item_id,
       (SELECT project_id
          FROM con_contract
         WHERE contract_id = t1.contract_id) project_id,

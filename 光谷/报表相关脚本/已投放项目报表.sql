SELECT rownum AS seq_number, -- 序号
       ...
       (SELECT to_char(cw.write_off_date, 'yyyy')
          FROM csh_write_off cw
         WHERE cw.contract_id = c.contract_id
           AND cw.cf_item = 0
           AND cw.write_off_due_amount <> 0) AS, -- 投放年份
       ...
       (SELECT u.description
          FROM sys_user u
         WHERE u.user_id = pp.coorganiser_user_id) coorganiser_user_id_n, --协办
       ...
       (SELECT hl.company_nature_n
         ...
         WHERE hl.bp_id = p.bp_id_tenant
           AND rownum = 1) company_nature_n, -- 企业性质
       ...
       (SELECT listagg(ci.short_name || ':' || ci.original_asset_value, ',') within GROUP(ORDER BY ci.contract_id) original_asset_value
          FROM con_contract_lease_item ci
       ...
       (SELECT rfc.description
         ...
         WHERE rfc.five_class_code = rft.five_class_code
           AND rft.contract_id = c.contract_id) five_class_name, -- 正常五级分类
       (SELECT v.code_value_name
         ...
         WHERE v.code = 'INTERNAL_RISK'
           ...
           AND rft.contract_id = c.contract_id) five_class_name_inner, -- 内部五级分类
       ...
       (SELECT h.credit_rating_n
          FROM hls_bp_master_lv h, con_contract_bp cb
         WHERE h.bp_id = cb.bp_id
           AND cb.contract_id = c.contract_id
           AND cb.bp_category = 'TENANT'
           AND rownum = 1
         ORDER BY h.rating_date DESC) tenant_credit_rating, -- 承租人最新评级（最高）  
       (SELECT h.rating_agency
          FROM hls_bp_master_lv h, con_contract_bp cb
         WHERE h.bp_id = cb.bp_id
           AND cb.contract_id = c.contract_id
           AND cb.bp_category = 'TENANT'
           AND rownum = 1
         ORDER BY h.rating_date DESC) tenant_credit_rating_agency, -- 承租人评级机构,取最新
       (SELECT h.rating_date
          FROM hls_bp_master_lv h, con_contract_bp cb
         WHERE h.bp_id = cb.bp_id
           AND cb.contract_id = c.contract_id
           AND cb.bp_category = 'TENANT'
           AND rownum = 1
         ORDER BY h.rating_date DESC) tenant_credit_rating_date, -- 承租人评级时间,取最新  
       (SELECT h.credit_rating_n
          FROM hls_bp_master_lv h, con_contract_bp cb
         WHERE h.bp_id = cb.bp_id
           AND cb.contract_id = c.contract_id
           ...
           AND rownum = 1
         ORDER BY h.rating_date DESC) guarantor_credit_rating, -- 担保人最新评级（最高）              
       (SELECT h.rating_agency
          FROM hls_bp_master_lv h, con_contract_bp cb
         WHERE h.bp_id = cb.bp_id
           AND cb.contract_id = c.contract_id
           AND cb.bp_category = 'GUARANTOR'
           AND rownum = 1
         ORDER BY h.rating_date DESC) guarantor_credit_rating_agency, -- 担保人评级机构,取最新
       (SELECT h.rating_date
          FROM hls_bp_master_lv h, con_contract_bp cb
         WHERE h.bp_id = cb.bp_id
           AND cb.contract_id = c.contract_id
           AND cb.bp_category = 'GUARANTOR'
           AND rownum = 1
         ORDER BY h.rating_date DESC) guarantor_credit_rating_date, -- 担保人评级时间,取最新 
       p.description, -- 项目简介
       p.creation_date, -- 项目创建日期
       (SELECT pm.meeting_name
          FROM prj_project_aproval pa, prj_project_meeting pm
         WHERE pa.project_id = p.project_id
           AND pa.meeting_id = pm.meeting_id) meeting_name, --立项会编号
       (SELECT z.last_update_date
          FROM zj_wfl_aprove_record z, zj_wfl_workflow_node_action za

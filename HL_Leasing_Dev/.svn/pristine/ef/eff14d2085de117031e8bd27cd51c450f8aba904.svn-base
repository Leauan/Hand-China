...
//							input.addClass('item-readOnly')
								...
								[ON]('click',function(){
									...
									if(svc){
								        ...
								        if(li!=-1){
								            ...
								            para_list = svc.substring(li+1,svc.length);
							...
							//              url = sf.context + 'sys_lov.screen?url='+encodeURIComponent(sf.context + 'sys_lov.svc?svc='+sf.lovservice + '&'+ Ext.urlEncode(sf.getLovPara()))+'&service='+sf.lovservice+'&';
							        	//alert(para);
							            //url = ctx+'sys_lov.screen?url='+encodeURIComponent(Ext.urlAppend(ctx + 'autocrud/'+svc+'/query',Ext.urlEncode(Ext.urlEncode(Ext.apply({},field.get('lovpara'),para_list)))))+'&service='+svc;
							        	url = ctx+'sys_lov.screen?url='+encodeURIComponent(Ext.urlAppend(ctx + 'autocrud/'+svc+'/query',para_list))+'&service='+svc;
							        }
							        if(url) {
...
//						input.set({id:[sf.id,name,record.id].join('_')});
		...
		onUpdate : function(ds,record, name, value){
			this.setValue(Ext.fly(this.id+'-'+record.id),record,name,value);
		},
	    ...
	    onValid : function(ds, record, name, valid){
	        ...
	        div && div[valid?'removeClass':'addClass'](ITEM_INVALID);
	    },
	    onLoad : function(){
